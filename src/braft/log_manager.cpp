...
static bvar::IntRecorder g_storage_append_entries_size("raft_storage_append_size");
...
void LogManager::append_to_storage(std::vector<LogEntry*>* to_append, 
    ...
    if (!_has_error.load(butil::memory_order_relaxed)) {
        ...
        if (written_size) {
            g_storage_append_entries_size << written_size;
            g_nomralized_append_entries_latency << timer.u_elapsed() * 1000 / written_size;
        }
    }
...
bvar::LatencyRecorder& LogManager::get_storage_append_entries_latency() {
    return g_storage_append_entries_latency;
}

bvar::IntRecorder& LogManager::get_storage_append_entries_written_size() {
    return g_storage_append_entries_size;
}
...
int LogManager::disk_thread(void* meta,
    ...
    for (; iter; ++iter) {
        ...
        if (!done->_entries.empty()) {
            // LOG(INFO) << "disk_thread append, last_id: " << last_id;
            ab.append(done);
        } else {
            // LOG(INFO) << "disk_thread ab.flush, last_id: " << last_id;
            ab.flush();
            int ret = 0;
            do {
...
void AppendBatcher::flush() {
    if (_size > 0) {
        // LOG(INFO) << "AppendBatcher::flush(), last_id: " << _last_id;
...
void AppendBatcher::append(LogManager::StableClosure* done) {
    if (_size == _cap || 
            _buffer_size >= (size_t)FLAGS_raft_max_append_buffer_size) {
        LOG(ERROR) << "_size: " << _size << ", _cap: " << _cap
            << ", _buffer_size: " << _buffer_size << ", FLAGS_raft_max_append_buffer_size" << FLAGS_raft_max_append_buffer_size;
        flush();
    }
    _storage[_size++] = done;
