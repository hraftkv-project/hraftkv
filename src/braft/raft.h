...
public:
    ...
    void set_bm_identifier(const std::string& value) { _bm_identifier = value; }
    std::string& get_bm_identifier() { return _bm_identifier; }

protected:
    butil::Status _st;
    std::string _bm_identifier;
};
...
// RS-Raft
struct RsTask {
    RsTask() : data(NULL), done(NULL), expected_term(-1) {}

    // The data to be replicated
    butil::IOBuf* metadata;

    // The data applied to performing RS encoding
    butil::IOBuf* data;
...
inline std::ostream& operator<<(std::ostream& os, const EntryInfo& e) {
    os << "EntryInfo {type=" << e.type 
       << ", k=" << e.k
       << ", fragment_index=" << e.fragment_index
       << ", fragment_len=" << e.fragment_len
       << ", valid_data_len=" << e.valid_data_len
    //    << ", data_len=" << e.data_len
    //    << ", metadata_len=" << e.metadata_len
       << "'}";
    return os;
}

// RS-Raft
// This struct is used by NodeImpl and Replicator class
struct MetadataAndClosure {
    butil::IOBuf data;
    butil::IOBuf metadata;
...
public:
    ...
    // Return the metadata whose content is the same as what was passed to
    // Node::apply in the leader node.
    const butil::IOBuf& metadata() const;

    // Return the entry type
    EntryInfo entry_info() const;
...
struct PeerStatus {
    PeerStatus()
        ...
        , readonly_index(0), consecutive_error_times(0), priority(0)
    {}
...
public:
    ...
    NodeStatus()
        ...
        , last_index(-1), disk_index(0), priority(0)
    {}
...
struct NodeOptions {
    ...
    PriorityManagerOptions* priority_manager_options;
...
public:
    ...
    // The RS-Raft version of apply
    // About the ownership:
    // |task.metadata|: for the performance consideration, we will take away the 
    //                  content. If you want keep the content, copy it before call
    //                  this function
    ...
    // RS-Raft recovery read entry point
    // About the ownership:
    // |task.metadata| and |task.data|: for the performance consideration, we will take away the 
    //                  content. If you want keep the content, copy it before call
    //                  this function
    // |meta|: We will take away the content as well.
    void collect_fragment(const RsTask& task, FragmentMeta& meta);
    ...
    const PriorityManager* priority_manager();
