...
public:
    ...
    virtual void sync();
protected:
    virtual scoped_refptr<Segment> open_segment();
    virtual int save_meta(const int64_t log_index);
    int load_meta();
    virtual int list_segments(bool is_empty);
    virtual int load_segments(ConfigurationManager* configuration_manager);
    virtual int get_segment(int64_t log_index, scoped_refptr<Segment>* ptr);
    virtual void pop_segments(
            int64_t first_index_kept, 
            std::vector<scoped_refptr<Segment> >* poped);
    virtual void pop_segments_from_back(
