...
// Rs-Raft
#include "braft/rs-raft/rs_log_storage.h"
...
DEFINE_int32(raft_rs_k, 3, "The k of RS encode");
...
NodeImpl::NodeImpl(const GroupId& group_id, const PeerId& peer_id)
    ...
    , _collection_box(NULL) // RS-Raft
    , _last_collect_index(0)
...
NodeImpl::~NodeImpl() {
    ...
    // RS-Raft
    if (_collect_fragment_queue) {
        _collect_fragment_queue->stop();
        _collect_fragment_queue.reset();
        bthread::execution_queue_join(_collect_fragment_queue_id);
    }
    if (_collection_box) {
        delete _collection_box;
    }
...
int NodeImpl::init_log_storage() {
    ...
    if (FLAGS_raft_log_storage_type == "rs_segment") {
        _raft_log_storage_type = LogStorageType::RS_SEGMENT;
        _log_storage = RsSegmentLogStorage::create(_options.log_uri);
        _log_manager = new LogManager();
        LOG(INFO) << "[RS] LogManager and RsSegmentLogStorage enabled.";
    } else {
        /**
         * Reminder: LogStorage::create can determine which subclass to use by
         * the protocol of log_uri, for example: 
         * local://<path> -> SegmentLogStorage
         * memory://<path> -> MemoryLogStorage
         * 
         * The association between the protocol and storage type is defined in
         * the GlobalExtension structure in raft.cpp.
         */
        _raft_log_storage_type = LogStorageType::NORMAL;
        _log_storage = LogStorage::create(_options.log_uri);
        _log_manager = new LogManager();
        LOG(INFO) << "Normal LogManager and SegmentLogStorage enabled.";
    }
...
int NodeImpl::init_priority_manager(const PriorityManagerOptions* options) {
    int ret = 0;
    if (options) {
        LOG(INFO) << "[DEBUG] PriorityManagerOptions is provided, priority: " << options->initial_priority;
        _priority_manager = new PriorityManager(this, _log_manager, *options);
    } else {
        LOG(INFO) << "[DEBUG] PriorityManagerOptions is not provided, use default one";
        PriorityManagerOptions default_options;
        _priority_manager = new PriorityManager(this, _log_manager, default_options);
    }
    _priority_manager->start();
    return ret;
}
...
int NodeImpl::bootstrap(const BootstrapOptions& options) {
     ...
     */
    if (init_priority_manager(_options.priority_manager_options) != 0) {
        LOG(ERROR) << "node " << _group_id << ":" << _server_id
                   << " init_priority_manager failed";
        return -1;
    }
    LOG(INFO) << _priority_manager->status_str();
...
int NodeImpl::init(const NodeOptions& options) {
    ...
    CHECK_EQ(0, _vote_timer.init(this, options.vote_timeout_ms));
    CHECK_EQ(0, _election_timer.init(this, options.election_timeout_ms));
    CHECK_EQ(0, _stepdown_timer.init(this, options.stepdown_timeout_ms));
    ...
    // RS-Raft
    if (FLAGS_raft_log_storage_type == "rs_segment") {
        // Create RS-Raft fragment collection box
        _collection_box = new CollectionBox();
        CollectionBoxOptions options;
        options.waiter = _fsm_caller;
        _collection_box->init(options);
    }
     ...
     * Dependency: LogManager
     */
    if (init_priority_manager(options.priority_manager_options) != 0) {
        LOG(ERROR) << "node " << _group_id << ":" << _server_id
                   << " init_priority_manager failed";
        return -1;
    }
    ...
    if (_priority_manager->is_ple_enabled()) {
        CHECK_EQ(0, _proba_vote_timer.init(this, options.vote_timeout_ms));
        _proba_vote_timer.set_mode(ProbationaryTimer::Mode::VOTE);
    }
    if (_priority_manager->is_pre_vote_ple_enabled()) {
        CHECK_EQ(0, _proba_pre_vote_timer.init(this, options.election_timeout_ms));
        _proba_pre_vote_timer.set_mode(ProbationaryTimer::Mode::PRE_VOTE);
    }
    ...
    rg_options.collection_box = _collection_box;
...
void NodeImpl::rs_apply(const RsTask& task, bool enable_coding) {
    LogEntry* entry = new LogEntry;
    entry->AddRef();
    // Swap only when the target is not null
    // otherwise SEGFAULT occur
    if (task.metadata != NULL) {
        entry->metadata.swap(*task.metadata);
    }
    if (task.data != NULL) {
        entry->data.swap(*task.data);
    }
    entry->type = enable_coding ? ENTRY_TYPE_RS_COMPLETE : ENTRY_TYPE_RS_REPLICATE;
...
void NodeImpl::shutdown(Closure* done) {
    ...
    {
        /**
         * Temp fix: somehow a thread for replicator has locked the _mutex and disable from quiting
         * we just simply unlock it first
         */
        _mutex.unlock();
        BAIDU_SCOPED_LOCK(_mutex);
        ...
        if (_state < STATE_SHUTTING) {  // Got the right to shut
             ...
             */
            if (_priority_manager->is_ple_enabled()) {
                _proba_vote_timer.destroy();
            }
            if (_priority_manager->is_pre_vote_ple_enabled()) {
                _proba_pre_vote_timer.destroy();
            }
 ...
 * <HRaft> - PreVote
 */
void NodeImpl::handle_proba_pre_vote_timeout() {
    std::unique_lock<raft_mutex_t> lck(_mutex);
    // stop the timeout first
    _proba_pre_vote_timer.stop();
    // check if ple enabled
    if (!_priority_manager->is_pre_vote_ple_enabled()) {
        LOG(ERROR) << "[PLE] Error, PLE in PreVote is not enabled but ProbationaryTimer is being used, please check.";
        return;
    }
    // check state
    if (_state != STATE_FOLLOWER) {
    ...
    /**
     * The ProbationaryTimer must be set when the node is probationary
     * If the node turns to be formal candidate now, a new leader 
     * with less or equal priority should have elected, skip it.
     */
    if (!_priority_manager->is_probationary()) {
        return;
    }
    /**
     * <HRaft>
     * Prioritized leader election - Election rules
     * Probationary candidate waits for receiving all response from 
     *  other servers until the election timeout elapses
     */
    if (_pre_vote_ctx.granted()) {
        LOG(INFO) << "[PLE] probationary candidate " << _group_id << ":" << _server_id
              << " receive pre vote from a majority of servers before probationary timeout, elect self";
        elect_self(&lck);
        return;
    }
}

void NodeImpl::handle_proba_vote_timeout() {
    std::unique_lock<raft_mutex_t> lck(_mutex);
    // stop the timeout first
    _proba_vote_timer.stop();
    // check if ple enabled
    if (!_priority_manager->is_pre_vote_ple_enabled()) {
        LOG(ERROR) << "[PLE] Error, PLE is not enabled but ProbationaryTimer is being used, please check.";
        return;
    }
    // check state
    ...
    if (_vote_ctx.granted()) {
        LOG(INFO) << "[PLE] probationary candidate " << _group_id << ":" << _server_id
              << " receive votes from a majority of servers before vote timeout, declare leader";
        become_leader();
        return;
    }
}
...
int NodeImpl::transfer_leadership_to(const PeerId& peer, bool call_from_rep) {
    LOG(INFO) << "[PLE DEBUG] NodeImpl::transfer_leadership_to, pass 1, call_from_rep: " << call_from_rep;
    // std::unique_lock<raft_mutex_t> lck(_mutex);
    if (!call_from_rep) {
        _mutex.lock();
    }
    LOG(INFO) << "[PLE DEBUG] NodeImpl::transfer_leadership_to, pass 2";
    ...
    LOG(INFO) << "[PLE DEBUG] NodeImpl::transfer_leadership_to, pass 3";

    /**
     * <HRaft>
     * Prioritized leader election - Leadership transfer
     * The leader checks the poiority of its followers and 
     *  select a best server to transfer its leadership (if exists)
     */
    if (_priority_manager->is_ple_enabled() && (peer_id == ANY_PEER)) {
        LOG(INFO) << "[PLE] node " << _group_id << ":" << _server_id
                  << " starts to look up for a best server.";

        if (_replicator_group.find_potential_best_server(
                &peer_id, _conf, _priority_manager->priority(), _options.catchup_margin) != 0) {
            LOG(INFO) << "[PLE] node " << _group_id << ":" << _server_id
                  << " no potential best server is found, or the node itself is a best server.";
            if (!call_from_rep) {
                _mutex.unlock();
            }
            return -1;
    ...
    LOG(INFO) << "[PLE DEBUG] NodeImpl::transfer_leadership_to, pass 4, peer_id: " << peer_id;
    ...
    LOG(INFO) << "[PLE DEBUG] NodeImpl::transfer_leadership_to, pass 5";
    ...
    LOG(INFO) << "[PLE DEBUG] NodeImpl::transfer_leadership_to, pass 6";
    ...
    _mutex.unlock();
    LOG(INFO) << "[PLE DEBUG] NodeImpl::transfer_leadership_to, pass 7";
    return 0;
}
...
void NodeImpl::handle_request_vote_response(const PeerId& peer_id, const int64_t term,
    ...
    } else if (_priority_manager->is_ple_enabled() &&
        (response.priority() > _priority_manager->priority())) {
        /**
         * <HRaft>
         * Prioritized leader election PLE - Election rules
         * If a probationary candidate receives decline vote from server
         *  with higher priority value, returns to follower state
         */
        LOG(INFO) << "[PLE] candidate " << _group_id << ":" << _server_id
                  << " receive decline vote from " << peer_id
                  << " with higher priorty: " << response.priority() << " than _priority: " << _priority_manager->priority()
    ...
    if (_priority_manager->is_ple_enabled()) { // Prioritized Leader Election
        /**
         * <HRaft>
         * Prioritized leader election PLE - Election rules
         * Separate the criteria for formal candidate and 
         *  probationary candidate to win an election
         */
        if (_priority_manager->is_probationary()) { // probationary candidate
            if (_vote_ctx.all_granted()) { // declare leader if receiving vote from all servers
                LOG(INFO) << "[PLE] probationary candidate " << _group_id << ":" << _server_id 
                    << " receives votes from all server, declare leader";
                _proba_vote_timer.stop();
                become_leader();
            } else if (_vote_ctx.granted()) { // continue to wait, even receiving votes from a majoirty of servers
                LOG(INFO) << "[PLE] probationary candidate " << _group_id << ":" << _server_id 
                    << " receives votes from a majoirty of servers, continue to wait";
            }
        } else if (_vote_ctx.granted()) { // formation candidate who acquire vote of majority, declare leader
            LOG(INFO) << "[PLE] formal candidate " << _group_id << ":" << _server_id 
                    << " receives votes from a majoirty of servers, declare leader";
            become_leader();
        }
    } else { // Original Leader Election
        if (_vote_ctx.granted()) {
            become_leader();
        }
    }
    ...
    // Deprecated
    // if (_ple_enabled) {
    //     if (!_priority_manager->is_probationary() && _vote_ctx.granted()) {
    //         LOG(INFO) << "[PLE] request_vote_response: !_priority_manager->is_probationary() && _vote_ctx.granted()" << 
    //         ", _leader_priority: " << _leader_priority << ", _priority: " << _priority;
    //         become_leader();
    //     } else if (_vote_ctx.granted()) {
    //         LOG(INFO) << "[PLE] request_vote_response: _vote_ctx.granted(), but I am a probationary node, wait" << 
    //         ", _leader_priority: " << _leader_priority << ", _priority: " << _priority;
    //     }
    // } else if (_vote_ctx.granted()) {
    //     become_leader();
...
void NodeImpl::handle_pre_vote_response(const PeerId& peer_id, const int64_t term,
    ...
    if (response.granted()) {
        ...
        response.priority() > _priority_manager->priority()) {
        LOG(INFO) << "[PLE] candidate " << _group_id << ":" << _server_id
                  << " receive decline pre vote from " << peer_id
                  << " with higher priorty: " << response.priority() << " than _priority: " << _priority_manager->priority()
                  << ", stop pre vote process";
        _proba_pre_vote_timer.stop(); // stop its pre vote.
        butil::Status status;
        step_down(response.term(), false, status);
    }

    if (_priority_manager->is_pre_vote_ple_enabled()) { // Prioritized Leader Election
        /**
         * <HRaft>
         * Prioritized leader election PLE - Election rules
         * Separate the criteria for formal candidate and 
         ...
         */
        if (_priority_manager->is_probationary()) { // probationary candidate
            if (_pre_vote_ctx.all_granted()) { // declare leader if receiving vote from all servers
                LOG(INFO) << "[PLE] probationary candidate " << _group_id << ":" << _server_id 
                    << " receives pre votes from all server, elect self";
                // stop the probationary timer if exist
                _proba_pre_vote_timer.stop();
                elect_self(&lck);
            } else if (_pre_vote_ctx.granted()) { // continue to wait, even receiving votes from a majoirty of servers
                LOG(INFO) << "[PLE] probationary candidate " << _group_id << ":" << _server_id 
                    << " receives pre votes from a majoirty of servers, continue to wait";
            }
        } else if (_pre_vote_ctx.granted()) { // formation candidate who acquire vote of majority, declare leader
            LOG(INFO) << "[PLE] formal candidate " << _group_id << ":" << _server_id 
                    << " receives votes from a majoirty of servers, elect self";
            elect_self(&lck);
        }
    } else { // Original Leader Election
        if (_pre_vote_ctx.granted()) {
...
void NodeImpl::pre_vote(std::unique_lock<raft_mutex_t>* lck) {
    ...
    for (std::set<PeerId>::const_iterator
         ...
         */
        if (_priority_manager->is_pre_vote_ple_enabled()) {
            done->request.set_priority(_priority_manager->priority());
            // start the probationary timer if this node is probationary
            if (_priority_manager->is_probationary()) {
                _proba_pre_vote_timer.start();
            }
        }
...
void NodeImpl::elect_self(std::unique_lock<raft_mutex_t>* lck) {
    ...
    for (std::set<PeerId>::const_iterator
         ...
         * <HRaft>
         * Prioritized leader election PLE - Election rules
         * The RequestVote RPC implements this restruction by including the information
         *  of the log and the priority value of the candidate
         */
        done->request.set_priority(_priority_manager->priority());
    ...
    if (_priority_manager->is_ple_enabled()) {
        if (!_priority_manager->is_probationary() && _vote_ctx.granted()) {
            LOG(INFO) << "[PLE] formal candidate " << _group_id << ":" << _server_id
                        << " receive votes from majority of servers, declare leader";
            become_leader();
        } else if (_priority_manager->is_probationary()) {
            if (_vote_ctx.all_granted()) {
                LOG(INFO) << "[PLE] probationary candidate " << _group_id << ":" << _server_id
                            << ", _leader_priority: " << _priority_manager->leader_priority() << ", _priority: " << _priority_manager->priority()
                            << " receive votes from all servers, declare leader";
                _proba_vote_timer.stop();
                become_leader();
            } else {
                _proba_vote_timer.start();
            }
        }
    } else if (_vote_ctx.granted()) {
        become_leader();
...
void NodeImpl::step_down(const int64_t term, bool wakeup_a_candidate, 
    ...
    /**
     * <HRaft>
     * Stopall probationary timer if exists
     */
    std::stringstream ss;
    ss << "[PLE] stop_down - stop all existing probationary timer, election_timer: ";
    _proba_pre_vote_timer.describe(ss, false);
    _proba_vote_timer.describe(ss, false);
    LOG(INFO) << ss.str();
    _proba_pre_vote_timer.stop();
    _proba_vote_timer.stop();
...
void NodeImpl::become_leader() {
    ...
    /**
     * <HRaft>
     * Update leader priority to my priority
     */
    _priority_manager->update_leader_priority(_priority_manager->priority());

    // RS-Raft
    _collection_box->clear_pending_tasks();
    _last_collect_index = 1;
    _collection_box->reset_pending_index(_last_collect_index);

#ifdef BM_BRAFT
    /**
     * Benchmark
     * Func: LeaderElection
     * Location: become_leader
     */
    Benchmark &bm = Benchmark::getInstance();
    LeaderElection* func = static_cast<LeaderElection*>(bm.at(OP_ELECTION));

    /**
     * The LeaderElection function has not been added, add one
     * , we reuse the same LeaderElection function
     */
    if (func == NULL) {
        func = new LeaderElection();
        bm.add(func, OP_ELECTION);
    }

    // Mark the timestamp for the moment become leader
    func->become_leader.mark();
    // Mart the priority of leader
    func->leader_priority = _priority_manager->priority();
    // Mark the term 
    func->end_term = _current_term;
#endif
}
...
void NodeImpl::apply(LogEntryAndClosure tasks[], size_t size) {
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        // Append to ballot box to wait for commit
...
int NodeImpl::handle_pre_vote_request(const RequestVoteRequest* request,
    ...
    do {
        ...
        if (_priority_manager->is_pre_vote_ple_enabled()) {
             ...
             * <HRaft>
             * Prioritized leader election PLE - Election rules
             * A voter grants its vote under either of the following conditions:
             * 1) the candidate's log is more up-to-date than the voter's (rule_1)
             * 2) the candidate's log is as up-to-date as the voter's and candidates's
             *    priority value is not smaller than the voter's (rule_2)
             */
            bool rule_1 = (LogId(request->last_log_index(), request->last_log_term()) > last_log_id);
            bool priority_ok = (request->priority() >= _priority_manager->priority());
            bool rule_2 = ((LogId(request->last_log_index(), request->last_log_term()) == last_log_id) && priority_ok);

            std::stringstream ple_log;
            ple_log << "[PLE] node " << _group_id << ":" << _server_id << " receive PreVote from " << request->server_id()
                << " local term: " << last_log_id.term << ", request term: " << request->last_log_term()
                << " local index: " << last_log_id.index << ", request index: " << request->last_log_index()
                << " local priority: " << _priority_manager->priority() << ", request priority: " << request->priority();
            if (rule_1) {
                ple_log << ", passed rule_1";
            } else if (rule_2) {
                ple_log << ", passed rule_2";
            } else {
                ple_log << ", failed on all rules";
            }
            LOG(INFO) << ple_log.str();

            granted = (rule_1 || rule_2);
        } else {
...
int NodeImpl::handle_request_vote_request(const RequestVoteRequest* request,
    ...
    do {
        ...
        if (_priority_manager->is_ple_enabled()) {
            ...
            std::stringstream ple_log;
            ple_log << "[PLE] node " << _group_id << ":" << _server_id << " receive RequestVote from " << request->server_id()
                << " local term: " << last_log_id.term << ", request term: " << request->last_log_term()
                << " local index: " << last_log_id.index << ", request index: " << request->last_log_index()
                << " local priority: " << _priority_manager->priority() << ", request priority: " << request->priority()
                << " _voted_id.is_empty(): " << _voted_id.is_empty();
            if (rule_1) {
                ple_log << ", passed rule_1";
            } else if (rule_2) {
                ple_log << ", passed rule_2";
            ...
            will_grant_vote = (rule_1 || rule_2);
        } else {
            will_grant_vote = (LogId(request->last_log_index(), request->last_log_term()) >= last_log_id);
        }
        
        // save
        if (will_grant_vote && _voted_id.is_empty()) {
            butil::Status status;
    ...
    response->set_term(_current_term);
    LOG(INFO) << "[PLE] server " << _group_id << ":" << _server_id << " receive RequestVote from " << request->server_id()
        << " vote granted: " << (request->term() == _current_term && _voted_id == candidate_id);
    response->set_granted(request->term() == _current_term && _voted_id == candidate_id);

    /**
     * <HRaft>
     * Prioritized leader election
     * Add priorityVlaue to RequestVote RPCs
     */
    response->set_priority(_priority_manager->priority()); 
...
void NodeImpl::handle_append_entries_request(brpc::Controller* cntl,
     ...
     * <HRaft>
     * Prioritized leader election - Election rules
     * The heartbeat messages are AppendEntries RPCs with the leader priority
     *  such that each follower can update its state accordingly
     */
    _priority_manager->update_leader_priority(request->leader_priority());

    /**
     * <HRaft>
     * Prioritized leader election
     * Add priorityValue to AppendEntries RPCs
     */
    response->set_priority(_priority_manager->priority());
    ...
    for (int i = 0; i < request->entries_size(); i++) {
        ...
        if (entry.type() != ENTRY_TYPE_UNKNOWN) {
            ...
            /**
             * RS-Raft
             * 1. Check whether the entry type belongs to RS
             * 2. Copy addition LogEntry information for RS-Raft
             * 3. Separate metadata and data
             */
            switch (log_entry->type) {
                case ENTRY_TYPE_RS_COMPLETE:
                case ENTRY_TYPE_RS_FRAGMENT:
                case ENTRY_TYPE_RS_REPLICATE: {
                    // Copy addition LogEntry information
                    log_entry->k = entry.k();
                    log_entry->fragment_index = entry.fragment_index();
                    log_entry->fragment_len = entry.fragment_len();
                    log_entry->valid_data_len = entry.valid_data_len(); 
                    // log_entry->data_len = entry.data_len();
                    // log_entry->metadata_len = entry.metadata_len();
                    // Separate metadata and data
                    // Note that the metadata is in front of data
                    data_buf.cutn(&log_entry->metadata, entry.metadata_len());
                    if (entry.data_len() > 0) {
                        data_buf.cutn(&log_entry->data, entry.data_len());
                    }
                    // log_entry->data.append(data_buf);
                    break;
                }
                case ENTRY_TYPE_CONFIGURATION: {
                    if (entry.peers_size() > 0) {
                        ...
                        LOG(ERROR) << "node " << _group_id << ":" << _server_id
                         << " receive ENTRY_TYPE_CONFIGURATION with peers_size == 0"
                         << " in term " << entry.term()
                         << " index " << index;
                    }
                    // Do not break here, as ENTRY_TYPE_CONFIGURATION also 
                    // retreive the data as ENTRY_TYPE_DATA
                }
                default: {          
...
int NodeImpl::increase_term_to(int64_t new_term, const butil::Status& status) {
    LOG(INFO) << "[PLE DEBUG] NodeImpl::increase_term_to: pass 1";
    BAIDU_SCOPED_LOCK(_mutex);
    LOG(INFO) << "[PLE DEBUG] NodeImpl::increase_term_to: pass 2";
    if (new_term <= _current_term) {
        return EINVAL;
    }
    LOG(INFO) << "[PLE DEBUG] NodeImpl::increase_term_to: pass 3";
...
void NodeImpl::get_status(NodeStatus* status) {
    ...
    /**
     * <HRaft>
     * Add priority of leader to NodeStatus
     */
    status->priority = _priority_manager->priority();
    ...
    case STAGE_STABLE:
        {
             ...
             * <HRaft>
             * Prioritized leader election - Leadership transfer
             * Note that if there is any ongoing membership change, the current leader
             *  postpone the leadership transfer until the membership change completes
             * 
             * Notes:
             * 1. When group configuration is changed, the follow path would be executed
             *    - NodeImpl::unsafe_register_conf_change()
             *    - _conf_cxt.start()
             *    - NodeImpl::on_caughtup()
             *    - NodeImpl::ConfigurationCtx::next_stage(), and _stage == STAGE_STABLE
             *      if the configuration change has been finished
             * 
             * 2. When election is done and local node invoked become_leader()
             *    - NodeImpl::become_leader()
             *    - _conf_cxt.flush()
             *    - NodeImpl::unsafe_apply_configuration()
             *    - ConfigurationChangeDone::Run()
             *    - NodeImpl::on_configuration_change_done
             *    - NodeImpl::ConfigurationCtx::next_stage(), and _stage == STAGE_STABLE
             *      if the configuration collected by new leader has committed
             * 
             * So in these two scenario, the followingcode will be executed when changes are done,
             * which is also the time that we should check whether there is any best server
             * to transfer the leadership. We put the code here.
             */
            LOG(INFO) << "NodeImpl::ConfigurationCtx::next_stage: STAGE_STABLE";
            if (_node->priority_manager()->is_leadership_transfer_enabled() 
                && (_node->_state == STATE_LEADER)) {
                LOG(INFO) << "[PLE] node " << _node->_group_id << ":" << _node->_server_id
                    << " enters STAGE_STABLE, find best server to transfer leadership";
                _node->transfer_leadership_to(ANY_PEER, true);
            }
...
void ProbationaryTimer::run() {
    switch (_mode) {
        case PRE_VOTE: {
            _node->handle_proba_pre_vote_timeout();
            break;
        }
        case VOTE: {
            _node->handle_proba_vote_timeout();
            break;
        }
        default: {
            LOG(ERROR) << "Unknown ProbationaryTimter mode: " << _mode << ", probably did not set mode";
        }
    }
}

int ProbationaryTimer::adjust_timeout_ms(int timeout_ms) {
    return random_timeout(timeout_ms);
}
