...
void SnapshotExecutor::do_snapshot(Closure* done) {
    ...
    if (_downloading_snapshot.load(butil::memory_order_relaxed)) {
        ...
        LOG(INFO) << "node " << _node->node_id() << "is loading a snapshot";
        return;
    }
    ...
    if (_saving_snapshot) {
        ...
        LOG(INFO) << "node " << _node->node_id() << "is saving a snapshot";
        return;
    }
...
int SnapshotExecutor::on_snapshot_save_done(
    ...
    if (ret == 0) {
        ...
        bm::SaveSnapshot* bm_func = static_cast<bm::SaveSnapshot*>(bm.at(bm::BenchmarkType::OP_SAVE_SNAPSHOT));
        if (bm_func) {
            LOG(INFO) << "<Benchmark> SnapshotExecutor::on_snapshot_save_done: update SaveSnapshot" 
                << "last_included_index=" << meta.last_included_index()
                << " last_included_term=" << meta.last_included_term(); 
            bm_func->set_last_included_term(_last_snapshot_term);
            bm_func->set_last_included_index(_last_snapshot_index);
        }
    }
    if (ret == EIO) {
