...
void Node::rs_apply(const RsTask& task, bool enable_coding) {
    _impl->rs_apply(task, enable_coding);
}
...
const PriorityManager* Node::priority_manager() {
    return _impl->priority_manager();
}
...
const butil::IOBuf& Iterator::metadata() const {
    return _impl->entry()->metadata;
}

EntryInfo Iterator::entry_info() const {
    LogEntry* entry = _impl->entry();
    EntryInfo info;
    info.k = entry->k;
    info.type = entry->type;
    info.fragment_index = entry->fragment_index;
    info.fragment_len = entry->fragment_len;
    info.valid_data_len = entry->valid_data_len;
    // info.data_len = entry->data_len;
    // info.metadata_len = entry->metadata_len;
    return info;
}
