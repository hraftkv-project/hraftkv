...
#include "braft/rs-raft/collection_box.h"        // CollectionBox
...
public:
     ...
     * Getter for _priority of this follower
     */
    static int64_t get_priority(ReplicatorId id);
    
    /**
     * <HRaft>
     * Prioritized leader election - Leadership transfer
     * Allow outsider to check whether this follower has catch up, 
     * used by ReplicatorGroup::find_potential_best_server for 
     * finding potential best server
     */
    static bool is_catchup(ReplicatorId id, int64_t max_margin);
...
private:
    ...
    int _prepare_entry(int offset, EntryMeta* em, butil::IOBuf* payload);
...
private:
    ...
    bool _has_succeeded;
    bool _rpc_send_failed;
    bool _rpc_need_recovery;
     ...
     * <HRaft>
     * Prioritized leader election - Election rules
     * In Braft, leader will hold a replicator object of each follower
     * Therefore, we add a "_priority" to each of them to 
     * represent the array "followerPriorities"
     * 
     * Initialize value: -1
     */
    int64_t _priority;
...
public:
     ...
     * <HRaft>     
     * Prioritized leader election - Leadership transfer
     * Find a best server in this group, which the current leader should
     * select it as target server transfer its leadership to it
     * 
     * Reminder: After searching a best server, this function would also check
     * whether the leader is also a best server. If the leader itself is a best server,
     * it would replace the target server to the leader to prevent unnecessary transfer.
     * 
     * PeerId peer_id [out]: pointer of PeerId
     * const ConfigurationEntry& conf [in]: current group configuration
     * 
     * @return 0 on success, -1 otherwise
     */
    int find_potential_best_server(PeerId* peer_id, const ConfigurationEntry& conf, 
