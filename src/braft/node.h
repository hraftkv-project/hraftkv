...
#include "braft/rs-raft/rs_raft.h"
#include "braft/rs-raft/collection_box.h"
 ...
 * <HRaft>
 */
class ProbationaryTimer : public NodeTimer {
public:
    enum Mode {
        UNKNOWN = 0,
        PRE_VOTE,
...
public:
    ...
    // RS-Raft
    // |task|: RsTask with task.data and task.metadata
    // |enable_coding|: If enable_coding is true, the entry correspond to the task 
    //                  will assigned with type ENTRY_TYPE_RS_COMPLETE
    //                  , else will be assigned with tyhpe ENTRY_TYPE_RS_REPLICATE
    void rs_apply(const RsTask& task, bool enable_coding);
     ...
     * <HRaft>
     */
    void handle_proba_pre_vote_timeout();
    void handle_proba_vote_timeout();
    ...
    /**
     * <HRaft>
     * In original Braft, to provide a manual leadership transfer feature
     *  that allow user to force the leader to transfer its leadership to a
     *  designated follower. 
     * We modified it to implement our leadership transfer feature.
     * 
     * Changes from original:
     *  - invoke ReplicatorGroup::find_potential_best_server instead when _ple_enabled is true
     *  - added a parameter "call_from_rep", which should be set to true when the fuction
     *    is invoked from replicator, to prevent double _mutex lock
     */
    int transfer_leadership_to(const PeerId& peer, bool call_from_rep = false);
     ...
     * <HRaft>
     */
    PriorityManager* priority_manager() { return _priority_manager; }

    /**
     * <HRaft>
     * A getter for other classes (such as Replicator) to get the
     * threshold index
     */
    bool get_catchup_margin() const { return _options.catchup_margin; }

    const LogStorageType& get_raft_log_storage_type() const {
        return _raft_log_storage_type;
    }
...
friend class butil::RefCountedThreadSafe<NodeImpl>;
    ...
    int init_priority_manager(const PriorityManagerOptions* options);
...
private:
     ...
     * <HRaft> - PreVote
     */
    ProbationaryTimer _proba_vote_timer;
    ProbationaryTimer _proba_pre_vote_timer;

    LogStorageType _raft_log_storage_type;
    ...
    // RS-Raft
    bthread::ExecutionQueueId<MetadataAndClosure> _collect_fragment_queue_id;
    bthread::ExecutionQueue<MetadataAndClosure>::scoped_ptr_t _collect_fragment_queue;
    CollectionBox* _collection_box;
    int64_t _last_collect_index;
