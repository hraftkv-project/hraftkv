...
public:
    ...
    /**
     * [PLE] - Election rules
     * Purpose: used by probationary candidate to determine 
     *      whether is should declare leader before election timeout
     * 
     * @return true if receiving votes for all servers and quorum for old configuration,
     *      we do not impose the restriction for old configuration, because some peers in
     *      old configuration maybe already fade out from the cluster
     */
    bool all_granted() const { return _all == 0 && _old_quorum <=0; }
private:
    ...
    /**
     * [PLE] - Election rules
     * Purpose: vote counter for probationary candidate,
     *      initialized by _peers sizes instead of _peers.size() / 2 + 1
     */
    int _all;
};
