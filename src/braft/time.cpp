...
double TimeVal::msec() const {
    return (double)(this->sec() * 1000);
}
...
double TagPt::usedTime() {
    return DEFAULT_MS ? usedTimeMs() : usedTimeS();
}

double TagPt::usedTimeS() {
    return (_endTv - _startTv).sec();
}

double TagPt::usedTimeMs() {
    return (_endTv - _startTv).msec();
}
